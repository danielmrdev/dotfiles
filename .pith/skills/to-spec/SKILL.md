...
The issue tracker and triage label vocabulary should have been provided to you — run the `skill` tool with `setup-matt-pocock-skills` if not. When a decision is unresolved, use the `ask_user_question` tool rather than presenting a plain-text options prompt.
...
The `skill:to-tickets` skill consumes this workspace. Sequential tickets integrate directly in it. Parallel tickets use temporary local lanes created from its latest commit, merge back into it, and clean up after verification. A ticket lane is complete only when its changes are in this integration branch; no ticket lane proposes or performs a merge into the default branch. Designate the final integration owner explicitly in the ticket map or a final follow-up ticket. Only that owner, after all tickets have landed and full integrated validation is green, may request approval to merge this integration branch into the default branch. After the approved merge, validate there, then remove the spec worktree and delete the local integration branch. Merging into the default branch and cleanup require the normal repository approvals.
...
- **Merge authority:** ticket lanes merge only into the integration branch; the integration owner requests the later default-branch merge
