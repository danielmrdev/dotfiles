...
The issue tracker and triage label vocabulary should have been provided to you — run the `skill` tool with `setup-matt-pocock-skills` if not. Use the `ask_user_question` tool for the approval questions below.
...
For every ticket, make the completion destination explicit before implementation starts:
- A parallel lane completes by validating its lane and integrating locally into the branch named by `Integrates into` in that ticket's `Workspace` section.
- A sequential ticket completes in the spec integration worktree and leaves that branch ready for the next ticket.
- Only the spec integration owner may propose the final merge into the repository default branch, after every ticket has landed and full integrated validation is green.

Do NOT close or modify any parent issue. Complete when every ticket is `ready-for-agent`, all tracker relationships are verified, every ticket has an unambiguous integration target, and the final visual map uses real IDs/links.
...
- To complete a parallel ticket: validate its lane, merge it locally into the exact branch named by `Integrates into`, validate that integration worktree, remove the lane worktree, and delete the local lane branch. Its completion handoff is to the spec integration branch.
- Integration and lane branches remain local and are never pushed.
- Designate one ticket or explicit follow-up as the integration owner. That owner performs full integrated verification and records a post-merge cleanup handoff. Only after that validation may the approved integration branch merge into the repository default branch. This default-branch merge is outside the completion of any individual parallel lane.
