...
- swapping to a **new harness** (Pith → another harness),
...
**5. Otherwise, compaction.** Relevant context, same harness, same directory, and you need to stay in the loop: this is where the tree lands, and it lands here often. Pass it an instruction (`/compact we're going to QA this area`) so the summary keeps what the next phase needs.
...
| Secondary (compaction, `/handoff`) | Lossy       | Less  | Lots         |
