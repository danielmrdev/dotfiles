...
BarWidget {
  ...
  WidgetButton {
    ...
    anchors.fill: parent
    bar: root.bar
    text: panelLoader.item ? panelLoader.item.label + (panelLoader.item.reportTempNum !== "" ? " " + panelLoader.item.reportTempNum + panelLoader.item.tempUnit : "") : ""
    // Text label (icon + temp), sized to content — no icon-slot clipping.
    labelVisible: true
